...
import { CompoundMove, BaseMove } from "./move.js";
...
export default class Game extends Schema {
  @type(Board) board: Board = new Board(this)
  //board = new Board(this);
  
  @type(Player) whitePlayer: Player = new Player("White", this)
  @type(Player) blackPlayer: Player = new Player("Black", this)
  @type(Player) winner: Player = null

  @type([ BaseMove ]) moves = new ArraySchema<BaseMove>()
  @type("string") result: GameResult = null;
  @type("string") terminationReason: GameTerminationReason = null;
  ...
  fromParsedToken(pgn: ParseTree) {
    ...
    pgn.moves.forEach((token) => {
      if (
        ...
        if (token.turn === "w") {
          castlingKingClass = this.whitePlayer.livePieceMap.get("King");
          
        } else {
          castlingKingClass = this.blackPlayer.livePieceMap.get("King");
        }
        castlingKing = castlingKingClass.pieces[0]
